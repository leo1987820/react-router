...
function createRouter(init) {
	...
	function getInstrumentationNavigateMeta(location, matches) {
		return {
	...
	async function startNavigation(historyAction, location, opts) {
		...
		opts?.instrumentationNavigateMetaReceiver?.(getInstrumentationNavigateMeta(location, matches || null));
		if (!matches) {
	...
	async function fetch(key, routeId, href, opts) {
		...
		if (fogOfWar.active && fogOfWar.matches) matches = fogOfWar.matches;
		if (!matches) {
			instrumentationResultMetaReceiver?.(getInstrumentationNavigateMeta(normalizedPath, null));
			setFetcherError(key, routeId, getInternalRouterError(404, { pathname: normalizedPath }), { flushSync });
			return;
		}
		instrumentationResultMetaReceiver?.(getInstrumentationNavigateMeta(normalizedPath, matches));
